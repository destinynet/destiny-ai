...
class GenerationPlanTest {
  ...
  // ============================================================
  // Usage / Cost 聚合測試（DefaultExecutionEngine 新增能力）
  // ============================================================

  @Test
  fun `test token usage aggregated across AI and parallel segments`() = runBlocking {
    val AI = SegmentId("ai")
    val PARALLEL = SegmentId("parallel")
    ...
    val plan = GenerationPlan(
      ...
      segments = listOf(
        Segment.AiSegment(
          id = AI,
          inputBuilder = { TestInput("go") },
          promptBuilder = { input, _ -> (input as TestInput).prompt },
          formatSpec = aiFormatSpec
        ),
        Segment.ParallelAiSegment<String, TestItemOutput>(
          id = PARALLEL,
          itemsProvider = { listOf("a", "b") },
          itemInputBuilder = { item, _ -> TestItemInput(item) },
          promptBuilder = { input, _ -> (input as TestItemInput).itemId },
          formatSpec = parallelFormatSpec
        )
      ),
      assembler = { "done" }
    ...
    // 每通 reply 帶固定 token（in=100 out=20 cacheCreate=10 cacheRead=5）
    val orchestrator = createMockOrchestratorWithUsage(
      inputTokens = 100, outputTokens = 20, cacheCreationTokens = 10, cacheReadTokens = 5
    ) { kClass ->
      when (kClass) {
        TestAiOutput::class -> TestAiOutput("hi", 1)
        TestItemOutput::class -> TestItemOutput("i", "r")
        else -> throw IllegalArgumentException("$kClass")
      }
    }

    // 無 modelCostService → costUsd 應為 null，但 token 仍累計
    val engine = DefaultExecutionEngine(
    ...
    val metadata = (engine.execute(plan) as ExecutionResult.Success).metadata

    // 3 通呼叫（1 AiSegment + 2 parallel items）
    assertEquals(3, metadata.totalAiCalls)
    ...
    assertEquals(15, usage.cacheReadTokens)
    assertEquals(null, metadata.costUsd, "未注入 ModelCostService，成本應為 null")
    // 同一 model 去重 → 只留一筆
    assertEquals(listOf(UsedModel("CLAUDE", "test-model")), metadata.modelsUsed)
  }

  @Test
  fun `test cost aggregated when ModelCostService provided`() = runBlocking {
    val AI = SegmentId("ai")
    val formatSpec = FormatSpec.of<TestAiOutput>("TestOutput", "Test AI output")
    ...
    val plan = GenerationPlan(
      ...
      segments = listOf(
        Segment.ParallelAiSegment<String, TestAiOutput>(
          id = AI,
          itemsProvider = { listOf("x", "y", "z") },
          itemInputBuilder = { item, _ -> TestInput(item) },
          promptBuilder = { input, _ -> (input as TestInput).prompt },
          formatSpec = formatSpec
        )
      ),
    ...
    val orchestrator = createMockOrchestratorWithUsage(
      inputTokens = 100, outputTokens = 20, cacheCreationTokens = null, cacheReadTokens = null
    ) { TestAiOutput("hi", 1) }

    // 每通 reply 固定計價 $0.002 → 3 通 = $0.006
    val costService = mockk<ModelCostService>()
    every { costService.cost(any<Reply.Normal<*>>()) } returns 0.002
    ...
    assertEquals(3, metadata.totalAiCalls)
    assertEquals(0.006, metadata.costUsd!!, 1e-9)
    assertEquals(300, metadata.tokenUsage!!.inputTokens)
    verify(exactly = 3) { costService.cost(any<Reply.Normal<*>>()) }
  }
  ...
  private fun createMockOrchestratorWithUsage(
    inputTokens: Int?,
    outputTokens: Int?,
