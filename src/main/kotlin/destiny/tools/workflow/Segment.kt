 ...
 * - ImageSegment: text-to-image 生圖區段
...
sealed class Segment {
  ...
  /**
   * 圖片生成區段 - 呼叫 IImageGeneration 生圖，輸出 [ImageOutput]
   *
   * 與 [AiSegment] 的差異：
   * - 無 formatSpec / outputParser：生圖沒有結構化 schema，回傳即圖片
   * - 無 inputBuilder：prompt 直接由 context 組成（通常吃上游某個文字 segment 的輸出）
   * - 不帶 model：同 [AiSegment]，model 由引擎注入的 image orchestrator 之 ResilientConfig 決定
   *
   * @property id 區段識別碼
   * @property dependsOn 依賴的 segment IDs
   * @property promptBuilder 從 context 建構 text-to-image prompt
   * @property options 生圖參數（張數 / 解析度 / 品質 / 長寬比）；provider 不支援的欄位由各 impl 忽略
   */
  data class ImageSegment(
    override val id: SegmentId,
    override val dependsOn: Set<SegmentId> = emptySet(),
    val promptBuilder: (context: SegmentContext) -> String,
